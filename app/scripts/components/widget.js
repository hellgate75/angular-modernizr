...
/**
 * @ngdoc directive
 * @name angularModernizrApp.directive:widget
 * @description
 * # widget
 */
angular.module('angularModernizrApp')
  .directive('widget', ['modernizrService', function(modernizrService) {
    return {
      ...
      controller: ['$window', '$scope', function($window, $scope) {
        if(!$scope.options || $scope.options.type !== 'widget') {
          throw 'Invalid options or invalid option type for the generic widget!!';
        }
        // var $ = $window.jQuery;
        ...
        if (!angular.element('link#widget').length) {
          angular.element('head').append(
            '<link id="widget" href="styles/widget.css" rel="stylesheet"/>'
          );
        }
        ...
        $scope.widgetOptions = {};
        $scope.addWidget = function(widgetDomName/*, widgetAttributes, options*/) {
          if (widgetDomName) {
            var widgetDomHandler = angular.element('#'+$scope.widgetId())[0];
            if (widgetDomHandler) {
              angular.element(widgetDomName).append(widgetDomHandler);
            }
          }
        };
        ...
        this.setValue = function(value) {
          if (value && value.type === 'widget' && angular.isArray(value.value)) {
